...
public class MouseLooker : MonoBehaviour {
	...
	public static MouseLooker mouseLooker;
	...
	void Update() {
		...
		if(GameController.gameController.isLockedMouse && !GameController.gameController.isExit) LookRotation ();
		
		if(Input.GetButtonDown("Cancel") ) {
			LockCursor (false);
			GameController.gameController.isExit = true;
		}
		// if the player fires, then relock the cursor
		if(GameController.gameController.isLockedMouse && !GameController.gameController.isExit){
			LockCursor (true);
		}
		...
		if(Input.GetButtonDown("Fire1") && GameController.gameController.isExit) {
			LockCursor (true);
			GameController.gameController.isExit = false;
		}
	}
